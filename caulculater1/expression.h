...
class Expression {
public:
  enum State { START, OPERATOR, INTEGER, DECIMAL_POINT, DECIMAL, FINISHED };
  enum Function { SIN, COS, TAN, ASIN, ACOS, SQRT, SQU };

  // 构造器
  Expression();

  void insert(char c); // 表达式的构建
  double calculate(const double &num1, const double &numu2,
                   const char &op); // 运算
