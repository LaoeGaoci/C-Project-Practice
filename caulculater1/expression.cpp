...
// 更新状态
void Expression::setState(State state) { this->state = state; }
...
// 获取表达式
QString Expression::getExpression() { return exprStr.replace("0-", "-"); }
...
// 根据运算符进行计算
double Expression::calculate(const double &num1, const double &num2,
                             const char &op) {
  switch (op) {
  case '+':
    return num1 + num2;
...
void Expression::updateStack(int opM = -1) {
  while (num.getSize() > 1 && !op.ifEmpty() && priority(op.top()) >= opM &&
         op.top() != '(') {
    // 将两个数从操作符弹出
    double num2 = num.pop();
    double num1 = num.pop();
